...
an [`fr_col()`](https://vthanik.github.io/tlframe/reference/fr_col.md)
spec:
...
| `group` | `NULL` | Spanning header group name (auto-creates a span via [`fr_spans()`](https://vthanik.github.io/tlframe/reference/fr_spans.md)) |
...
![Decimal-aligned mixed statistics (counts, means, ranges,
p-values)](figures/preview_decimal.png)

Decimal-aligned mixed statistics (counts, means, ranges, p-values)
...
side is the
[`fr_col()`](https://vthanik.github.io/tlframe/reference/fr_col.md) to
apply:
...
[`fr_col()`](https://vthanik.github.io/tlframe/reference/fr_col.md) is a
...
`bold`, `bg`, `fg`, `font_size`, `repeat_on_page` (default `TRUE` — set
`FALSE` to print the header only on the first page).
...
> Batch](https://vthanik.github.io/tlframe/articles/automation.md).
...
> headers, then manual `COMPUTE` blocks to attach them. tlframe does it
> in one
> [`fr_cols()`](https://vthanik.github.io/tlframe/reference/fr_cols.md)
> call.
...
![Single-level spanning header grouping two dose arms under
'Zomerane'](figures/preview_spans.png)

Single-level spanning header grouping two dose arms under ‘Zomerane’
...
|----|----|
| `fr_col(group = "label")` | Inline shorthand; you are already naming every column in [`fr_cols()`](https://vthanik.github.io/tlframe/reference/fr_cols.md) |
...
> with `STYLE(SPANNING)`. tlframe handles all positioning automatically
> from a single
> [`fr_spans()`](https://vthanik.github.io/tlframe/reference/fr_spans.md)
> call.
...
![Column split panels with stub column
repeating](figures/preview_col_split.png)

Column split panels with stub column repeating
