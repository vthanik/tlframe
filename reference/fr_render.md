...
The terminal verb in every **tlframe** pipeline. Takes a fully
...
- path:
  ...
  - `.pdf` — PDF via XeLaTeX compilation (requires XeLaTeX on PATH or
    **tinytex**). First-time setup: call
    ...
    `TLFRAME_FONT_DIR` environment variable (see
    [`fr_page()`](https://vthanik.github.io/tlframe/reference/fr_page.md)).

- format:

  Character scalar or `NULL`. Output format: `"rtf"`, `"latex"`, or
  `"pdf"`. If `NULL` (default), detected from the file extension of
  `path`.
