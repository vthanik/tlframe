...
path is given, auto-discovers the nearest `_tlframe.yml` by searching up
...
[`fr_page()`](https://vthanik.github.io/tlframe/reference/fr_page.md) or
...
- file:
  ...
  Character scalar or `NULL`. Path to a `_tlframe.yml` file. `NULL`
  (default) triggers auto-discovery: searches up the directory tree from
  [`getwd()`](https://rdrr.io/r/base/getwd.html) until a `_tlframe.yml`
  is found, falling back to the package-bundled defaults
  (`inst/defaults/_tlframe.yml`).
...
Config from `_tlframe.yml` overrides package defaults.
[`fr_theme()`](https://vthanik.github.io/tlframe/reference/fr_theme.md)
