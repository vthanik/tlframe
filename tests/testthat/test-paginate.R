...
# ══════════════════════════════════════════════════════════════════════════════
# Group continuation headers
# ══════════════════════════════════════════════════════════════════════════════

test_that("group_cont parameter stored in spec$body", {
  spec <- tbl_demog |>
    fr_table() |>
    fr_rows(group_by = "group", group_cont = "(continued)")
  expect_equal(spec$body$group_cont, "(continued)")
})

test_that("group_cont = NULL by default", {
  spec <- tbl_demog |>
    fr_table() |>
    fr_rows(group_by = "group")
  expect_null(spec$body$group_cont)
})

test_that("group_cont validates as character scalar", {
  expect_error(
    tbl_demog |>
      fr_table() |>
      fr_rows(group_cont = 42),
    "group_cont"
  )
})

test_that("group_cont wired through theme", {
  withr::with_options(list(), {
    fr_theme_reset()
    fr_theme(group_cont = "(cont.)")
    spec <- tbl_demog |> fr_table()
    expect_equal(spec$body$group_cont, "(cont.)")
    fr_theme_reset()
  })
})

test_that("group_cont RTF output contains continuation header", {
  # Build a table with enough rows to force a page split within a group
  # Create a large group that will span pages
  n_rows <- 80L
  df <- data.frame(
    soc = rep("Cardiac disorders", n_rows),
    pt = paste("PT", seq_len(n_rows)),
    n = as.character(seq_len(n_rows)),
    stringsAsFactors = FALSE
  )
  ...
  df |>
    fr_table() |>
    fr_rows(group_by = "soc", indent_by = "pt", group_cont = "(continued)") |>
    fr_render(out)

  rtf <- readLines(out)
  rtf_text <- paste(rtf, collapse = "\n")
  # Should contain "(continued)" in the output
  expect_true(grepl("continued", rtf_text, fixed = TRUE))
})

test_that("group_cont not emitted when groups don't split", {
  # Small table that fits on one page — no continuation headers needed
  df <- data.frame(
    soc = c("Eye disorders", "Eye disorders", "Eye disorders"),
    pt = c("Eye disorders", "Conjunctivitis", "Dry eye"),
    n = c("10", "6", "4"),
    stringsAsFactors = FALSE
  )
  out <- tempfile(fileext = ".rtf")
  ...
  rtf <- readLines(out)
  rtf_text <- paste(rtf, collapse = "\n")
  # No continuation needed — content fits on one page
  expect_false(grepl("continued", rtf_text, fixed = TRUE))
})

test_that("group_cont NULL produces no continuation rows", {
  n_rows <- 80L
  df <- data.frame(
    soc = rep("Cardiac disorders", n_rows),
    pt = paste("PT", seq_len(n_rows)),
...
test_that("budget reduced by 1 when group_cont is set", {
  spec <- tbl_ae_soc |>
    fr_table() |>
    fr_rows(group_by = "soc", group_cont = "(continued)")

  budget_with <- calculate_page_budget(spec) - 1L
  spec2 <- tbl_ae_soc |>
    fr_table() |>
    fr_rows(group_by = "soc")
  budget_without <- calculate_page_budget(spec2)

  # Same base budget (calculate_page_budget doesn't know about group_cont;

  # the -1 is applied in the render loop)
  expect_equal(budget_with + 1L, budget_without)
})

test_that("group_cont with multiple group_by columns", {
  n_rows <- 80L
  df <- data.frame(
    soc = rep("Cardiac disorders", n_rows),
    severity = rep(c("Mild", "Severe"), length.out = n_rows),
    pt = paste("PT", seq_len(n_rows)),
    n = as.character(seq_len(n_rows)),
    stringsAsFactors = FALSE
  )
  ...
  # Should not error with multiple group_by columns
  expect_no_error(
    df |>
      fr_table() |>
      fr_rows(
        group_by = c("soc", "severity"),
        group_cont = "(continued)"
      ) |>
      fr_render(out)
  )
})
