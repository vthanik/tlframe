...
fr_validate(ae_listing_spec)$valid
```
...
spec <- data.frame(x = 1) |>
  fr_figure(plot = my_plot, width = 8, height = 5) |>
  fr_titles(
    "Figure 14.2.1",
...
|-----------|---------|-------------|
| `plot` | `NULL` | ggplot2 object or recorded plot |
| `width` | `NULL` | Plot width in inches |
| `height` | `NULL` | Plot height in inches |
